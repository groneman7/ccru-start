import { pgTable, pgSchema, foreignKey, uuid, text, timestamp, unique, integer, boolean, time, pgEnum } from "drizzle-orm/pg-core"
...
export const eventsInCalendar = calendar.table("events", {
...
export const junctionTemplatePositionsInCalendar = calendar.table("junction_template_positions", {
	id: uuid().default(sql`uuid_generate_v7()`).primaryKey().notNull(),
...
export const userInBetterAuth = betterAuth.table("user", {
	...
	timestampCreatedAt: timestamp("timestamp_created_at", { mode: 'string' }).defaultNow().notNull(),
...
export const positionsInCalendar = calendar.table("positions", {
	id: uuid().default(sql`uuid_generate_v7()`).primaryKey().notNull(),
	name: text().notNull(),
	display: text().notNull(),
	description: text(),
}, (table) => [
	unique("positions_name_key").on(table.name),
]);

export const userTypesInAuthz = authz.table("user_types", {
	id: uuid().default(sql`uuid_generate_v7()`).primaryKey().notNull(),
	name: text().notNull(),
	display: text().notNull(),
}, (table) => [
...
export const templatesInCalendar = calendar.table("templates", {
	id: uuid().default(sql`uuid_generate_v7()`).primaryKey().notNull(),
	name: text().notNull(),
	display: text().notNull(),
	description: text(),
	...
	unique("templates_name_key").on(table.name),
]);

export const junctionShiftsInCalendar = calendar.table("junction_shifts", {
	id: uuid().default(sql`uuid_generate_v7()`).primaryKey().notNull(),
...
export const certificationsInAuthz = authz.table("certifications", {
	id: uuid().default(sql`uuid_generate_v7()`).primaryKey().notNull(),
	name: text().notNull(),
	display: text().notNull(),
}, (table) => [
	unique("certifications_name_key").on(table.name),
]);
...
export const junctionSlotsInCalendar = calendar.table("junction_slots", {
	id: uuid().default(sql`uuid_generate_v7()`).primaryKey().notNull(),
